...
const DIFY_API_KEY = process.env.DIFY_HOME_SUPPORT_API_KEY;
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("Home support workflow request:", {
      endpoint: ENDPOINT,
      query,
      conversationId,
    });
    ...
    if (!response.ok) {
      ...
      console.error("Dify home support workflow API error:", errorText);
      return new Response(
        JSON.stringify({
    ...
    console.error("Home support workflow API error:", error);
    return new Response(
      JSON.stringify({
